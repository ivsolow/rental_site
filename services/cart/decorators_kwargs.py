...
from services.cart.swagger_examples import (CART_LIST_RESPONSE,
                                            CART_CREATE_REQUEST,
...
CART_LIST_DECORATOR_KWARGS = {
    "description": "Retrieve all cart items from the database. "
                   "If different cart instances have the same"
                   " equipment and dates, they will be combined"
                   " into a single entry.",
...
CART_CREATE_DECORATOR_KWARGS = {
    "description": "When adding a new equipment to the cart,"
                   " a validation check is performed: if an"
                   "item with the same equipment and dates"
                   " already exists, the existing object will be updated."
    ...
    "examples": [
        ...
        OpenApiExample(
            name="Cart response for an existing"
                 "cart object with updated amount",
...
CART_DESTROY_DECORATOR_KWARGS = {
    "description": "Deletes a cart item or the entire "
                   "cart object, depending on the "
                   "amount of deleting equipment.",
